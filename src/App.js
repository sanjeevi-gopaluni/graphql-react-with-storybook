...
import { PRODUCT_BY_ITEM_KEY, GET_PRODUCTS,PRODUCT_UPSERT_SUBS,PRODUCT_DELETE_SUBS } from './config/queryConstants';
...
function App() {
  ...
  const delres = useSubscription(PRODUCT_DELETE_SUBS ,{
    onSubscriptionData: ({ subscriptionData: { data } }) => {
      console.log("PRODUCT_DELETED_SUBS",data);
      refetch();
    }
  }, )
  ...
  const renderCard = () => {
    if (!isEmpty(prdData) && !isEmpty(prdData.products)) {
      return <section className={"paddingTop2"}>
        {renderModal()}
        <div className={"displayFlex"}>
          <Grid container spacing={2}>
            {prdData.products.map(prd => <Grid item xs={6} sm={3}><ProductTileCard product={prd} refresh={() => { refetch() }} /></Grid>)}
          </Grid>
        </div>
